...
public class JOptionDepositDialog {
	CoinPad coinPad;
	SpringLayout layout;
	JPanel frame;
	...
	public JOptionDepositDialog(CoinPad coinPad) {
		...
		messageLabel1 = new JLabel("Choose the coin and amount you wish to deposit.");
	...
	public void showDialog(){		
		...
		int result = JOptionPane.showConfirmDialog(coinPad, frame, "Deposit", 
				JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
		
		if (result == JOptionPane.OK_OPTION){
			String coin = comboBox1.getSelectedItem().toString();
			float fee = Float.valueOf(textField2.getText());
			float amount = Float.valueOf(textField1.getText()) * ((100.0f-fee)/100.0f);
			float price = -1.0f;
			float capital = coinPad.db.getCoinCapital(coin) + amount;
			
			//update database
			coinPad.db.insertCapitalRow(coin, "D", amount);
			coinPad.db.insertInHistory(coin, "D", amount, price, fee, capital, -0.0f);
			
			//update gui
			coinPad.addHistoryTableRow(DateTime.now().toString(), coin, amount, price, fee, "D", capital, -0.0f);
